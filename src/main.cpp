...
class Mp3Notify {
   public:
    ...
    static void PrintlnSourceAction(DfMp3& mp3, DfMp3_PlaySources source, const char *action) {
    ...
    static void OnPlaySourceOnline(DfMp3& mp3, DfMp3_PlaySources source) {
        PrintlnSourceAction(mp3, source, "online");
    }
    static void OnPlaySourceInserted(DfMp3& mp3, DfMp3_PlaySources source) {
        PrintlnSourceAction(mp3, source, "ready");
    }
    static void OnPlaySourceRemoved(DfMp3& mp3, DfMp3_PlaySources source) {
        PrintlnSourceAction(mp3, source, "removed");
    }
};
...
// Play LastCard
void setupPlayShortcut(folderSettings *theFolder) {
...
void adminMenu(bool fromCard = false) {
    ...
    int subMenu = voiceMenu(14, 900, 900, false, 0, 0, true);
    if (subMenu == Exit) {
        digitalWrite(StatusLEDRedPin, LOW);
        analogWrite(StatusLEDGreenPin, ledMaxBrightness);
        return;
    ...
    if (subMenu == ResetCard) {
        ...
        // Maximum Volume
        mySettings.maxVolume = voiceMenu(30 - mySettings.minVolume, 930, mySettings.minVolume, false, 0, mySettings.maxVolume - mySettings.minVolume) + mySettings.minVolume;
    } else if (subMenu == MinVolume) {
        // Minimum Volume
        mySettings.minVolume = voiceMenu(mySettings.maxVolume - 1, 931, 0, false, 0, mySettings.minVolume);
    } else if (subMenu == InitVolume) {
        // Initial Volume
        mySettings.initVolume = voiceMenu(mySettings.maxVolume - mySettings.minVolume + 1, 932, mySettings.minVolume - 1, false, 0, mySettings.initVolume - mySettings.minVolume + 1) + mySettings.minVolume - 1;
    } else if (subMenu == EQ) {
        // EQ
        mySettings.eq = voiceMenu(6, 920, 920, false, 0, mySettings.eq);
        set_equlizer(mySettings.eq);
    ...
    // lock admin menu
        ...
        do {
            ...
            mp3.playMp3FolderTrack(400);
        } while (voiceMenu(2, 946, 933, false, 0, 1, true) == 2);
#endif
